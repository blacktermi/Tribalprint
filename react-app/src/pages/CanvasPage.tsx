...
const FORMATS = [
  { code: 'A2', group: 'Moyen format', label: 'Format A2 (42 × 60 cm)', price: 50000, widthCm: 42, heightCm: 60 },
  { code: 'A2S', group: 'Moyen format', label: 'Format A2 Carré (42 × 42 cm)', price: 50000, widthCm: 42, heightCm: 42 },
  { code: 'A1', group: 'Grand format', label: 'Format A1 (60 × 85 cm)', price: 80000, widthCm: 60, heightCm: 85 },
  { code: 'A1S', group: 'Grand format', label: 'Format A1 Carré (60 × 60 cm)', price: 80000, widthCm: 60, heightCm: 60 },
  { code: 'A0', group: 'Très grand format', label: 'Format A0 (85 × 119 cm)', price: 100000, widthCm: 85, heightCm: 119 },
  { code: 'A0S', group: 'Très grand format', label: 'Format A0 Carré (85 × 85 cm)', price: 100000, widthCm: 85, heightCm: 85 },
] as const
...
export default function CanvasPage() {
  ...
  const initialShapes = useMemo(() => Object.fromEntries(FORMATS.map(f => [f.code, (f.widthCm === f.heightCm ? 'square' : 'rect')])) as ShapesByFormat, [])
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <div>
          <div className="overflow-hidden rounded-xl border border-slate-200">
            <img src="/img/canvas-cover.jpg" alt="Canvas" className="w-full object-cover" />
          </div>
        </div>
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Canvas</h1>
          <p className="mt-1 text-slate-600 text-sm">Sélectionnez vos formats, importez vos photos. Remise 10% dès 5 tableaux, acompte 30% au-delà de 20 000 FCFA. Grands formats livrés via Yango.</p>
            ...
            <div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {selectedStandard.map((f) => {
                  ...
                  const isSquareFormat = f.widthCm === f.heightCm
                  return (
                    <div key={f.code} className={`rounded-lg border ${qty > 0 ? 'border-slate-900' : 'border-slate-200'} p-3`}>
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          {isSquareFormat ? (
                            <div className="mt-1 text-xs text-slate-700">Version: Carré</div>
                          ) : (

        ...
        <div>
          ...
          <div className="mt-6 space-y-6">
            ...
            <div className="flex flex-wrap gap-3 items-center">
              <button type="button" onClick={() => { setTouched(true); if (!formValid) return; navigate(confirmationTo) }} disabled={!formValid} className={`inline-flex items-center gap-2 rounded-full px-5 py-2 text-sm font-medium text-white ${formValid ? 'bg-slate-900 hover:bg-slate-800' : 'bg-slate-300 cursor-not-allowed'}`}>Commander</button>
            </div>
          </div>
        </div>
      </div>

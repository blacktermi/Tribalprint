...
export default function PlaquettePage() {
  ...
  const [coverSrc, setCoverSrc] = useState('/img/plaquette-cover.jpg')
  ...
  }, [pelliculage, qty, unitPrice, subtotal, discountAmount, delivery, total, zone, commune, fullName, phone, email, file, deliveryInfo.iso, deliveryInfo.window])
  ...
  async function handleOrder() {
    ...
    try {
      ...
      const payload = buildOrderPayload({
        ...
        items: [
          {
            product_slug: 'plaquette',
            product_label: 'Plaquette',
            quantity: qty,
            unit_price: unitPrice,
            options: { format: 'A4', pelliculage },
          },
        ],
      ...
      console.error('createOrder failed (plaquette), fallback:', e)
      navigate(confirmationTo)
    } finally {
      setSubmitting(false)
    }
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <div>
          <div className="overflow-hidden rounded-xl border border-slate-200">
            <img src={coverSrc} onError={() => setCoverSrc('/img/plaquette-cover.svg')} alt="Plaquette" className="w-full object-cover" />
          </div>
        </div>
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Plaquette</h1>
          <p className="mt-1 text-slate-600 text-sm">Format A4. Prix unitaire: 2 500 FCFA. Pelliculage mat ou brillant. Minimum 10 exemplaires.</p>

          <div className="mt-6 space-y-6">
            <div className="grid gap-4 md:grid-cols-3">
              <div className="md:col-span-2">
                <label className="text-sm font-medium">Nom et Prénom <span className="text-red-600">*</span></label>
                <input type="text" className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" value={fullName} onChange={(e) => setFullName(e.target.value)} onBlur={() => setTouched(true)} placeholder="Ex: Soro Ibrahim" />
              ...
              <div>
                ...
                <input type="tel" className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" value={phone} onChange={(e) => setPhone(e.target.value)} onBlur={() => setTouched(true)} placeholder="Ex: +225 01 23 45 67 89" />
            ...
            <div className="flex items-center gap-3">
              ...
              <input type="number" min={minQty} step={1} className="w-28 rounded-md border border-slate-300 px-3 py-2 text-sm" value={qty} onChange={(e) => setQty(Math.max(minQty, parseInt(e.target.value) || minQty))} />
            ...
            <div className="rounded-xl border border-slate-200 p-4 text-sm">
              <div className="flex justify-between"><span>{qty} × A4 — {pelliculage}</span><span>{subtotalItems.toLocaleString()} FCFA</span></div>

  ...
  async function handleOrder() {
    ...
    try {
      ...
      const payload = buildOrderPayload({
        ...
        items: [{ product_slug: 'carte-invitation', product_label: 'Cartes d\'invitation', quantity: packs * 100, unit_price: unitPackPrice / 100, options: { format, impression, pelliculage, bords_arrondis: bordsArrondis, packs } }],
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <div>
          <div className="overflow-hidden rounded-xl border border-slate-200">
            <img src={coverSrc} onError={() => setCoverSrc('/img/carte-invitation-cover.svg')} alt="Cartes d'invitation" className="w-full object-cover" />
          </div>
        </div>
        <div>
          ...
          <div className="mt-6 space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {(['24x14','14x10'] as const).map(f => (
                <button key={f} type="button" onClick={() => setFormat(f)} className={`rounded-full border px-3 py-1.5 text-sm ${format === f ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-300 hover:border-slate-400'}`}>{f.replace('x', ' × ')} cm</button>
              ))}
            </div>

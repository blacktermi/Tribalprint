...
export default function PostersPage() {
  ...
  const handleOrder = async () => {
    ...
    FORMATS.forEach((format) => {
      ...
      const shape = shapesByFormat[format.code]
      const squareSize = Math.min(format.widthCm, format.heightCm)
      const widthCm = shape === 'square' ? squareSize : format.widthCm
      const heightCm = shape === 'square' ? squareSize : format.heightCm
      const item: OrderItemInput = {
        product_slug: 'posters',
        product_label: 'Poster',
        quantity,
    ...
    customItems.forEach((it, idx) => {
      ...
      const item: OrderItemInput = {
        product_slug: 'posters',
        product_label: 'Poster personnalisé',
        quantity,
    ...
    try {
      ...
      if (res.ref) params.set('ref', res.ref)
      navigate(url.pathname + '?' + params.toString())
    } catch (error) {
      console.error('createOrder failed (posters), fallback:', error)

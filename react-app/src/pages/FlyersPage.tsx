...
const UNIT_PRICES: Record<Format, Record<Impression, number>> = {
  A5: { recto: 200, 'recto-verso': 250 },
  A4: { recto: 400, 'recto-verso': 500 },
}
...
export default function FlyersPage() {
  ...
  const minQty = 50
  const unitPrice = UNIT_PRICES[format][impression]
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          <p className="mt-1 text-slate-600 text-sm">Formats: A5 ou A4. Recto simple ou recto-verso. Minimum 50 ex.</p>

          <div className="mt-6 space-y-6">
            <div className="grid gap-4 md:grid-cols-3">
              <div className="md:col-span-2">
                <label className="text-sm font-medium">Nom et Prénom <span className="text-red-600">*</span></label>
                <input type="text" className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" value={fullName} onChange={(e) => setFullName(e.target.value)} onBlur={() => setTouched(true)} placeholder="Ex: Kouamé Michel" />
              ...
              <div>
                ...
                <input type="tel" className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" value={phone} onChange={(e) => setPhone(e.target.value)} onBlur={() => setTouched(true)} placeholder="Ex: +225 07 00 11 22 33" />
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {(['A5','A4'] as const).map(f => (
                <button key={f} type="button" onClick={() => setFormat(f)} className={`rounded-full border px-3 py-1.5 text-sm ${format === f ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-300 hover:border-slate-400'}`}>{f}</button>
              ))}
            </div>
            ...
            <div className="rounded-xl border border-slate-200 p-4 text-sm">
              <div className="flex justify-between"><span>{qty} × {format} {impression === 'recto' ? 'recto simple' : 'recto-verso'}</span><span>{subtotalItems.toLocaleString()} FCFA</span></div>

...
export default function TableauxAluminiumPage() {
  ...
  const onCustomFilesSelected = (id: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCustomItems(prev => prev.map(it => {
      ...
      imgs.forEach(f => map.set(key(f), f))
      return { ...it, photos: Array.from(map.values()) }
    }))
    e.currentTarget.value = ''
  }
  const removeCustomPhoto = (id: string, idx: number) => {
    setCustomItems(prev => prev.map(it => it.id === id ? { ...it, photos: it.photos.filter((_, i) => i !== idx) } : it))
  }
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          <p className="mt-1 text-slate-600 text-sm">Choisissez les formats désirés, importez vos photos pour chaque format, et renseignez vos options et coordonnées.</p>
                    ...
                    <div key={it.id} className="rounded-lg border border-slate-200 p-3">
                      ...
                      <div className="mt-3 grid gap-3 sm:grid-cols-5">
                        <div>
                          ...
                          <input type="number" min={1} step="0.1" className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1 text-sm" value={it.width} onChange={(e) => updateCustomField(it.id, 'width', e.target.value === '' ? '' : Number(e.target.value))} />
                        </div>
                        <div>
                          <label className="text-xs">Hauteur (cm) *</label>
                          <input type="number" min={1} step="0.1" className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1 text-sm" value={it.height} onChange={(e) => updateCustomField(it.id, 'height', e.target.value === '' ? '' : Number(e.target.value))} />
                        </div>
                        <div>
